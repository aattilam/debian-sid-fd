...
sudo -u $SUDO_USER touch /tmp/postinst.sh
sudo -u $SUDO_USER cat > /tmp/postinst.sh << EOF
#!/bin/bash
...
rm /tmp/postinst.sh
EOF
sudo -u $SUDO_USER chmod +x /tmp/postinst.sh
sudo -u $SUDO_USER echo "/tmp/postinst.sh" >> ~/.bashrc
